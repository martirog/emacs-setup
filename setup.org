...
** transparent frame
Not sure this is something that I ever want to use but it is cool sometimes so leave it here for later
the alpha-background not working at the moment for me althow this is prefered as the tests will not be transparent
...
** Wrapper function to start fido mode for some functions

TODO figure out if it is needed to disable reenable ido
#+BEGIN_SRC emacs-lisp
  (defun wrap-fido-mode-for-this-function (orig-func &rest args)
    "wrap any function to locally start fido-mode for this function"
    ;;do we need check for ido-mode and disabling if active?
    (message "setting fido mode")
    (fido-mode t)
    (message "fido mode %s" (fido-mode))
    (apply orig-func args)
    (fido-mode -1))
    (message "removing fido mode")
  ;; reenabling ido if active??
#+END_SRC
...
* version control
** disable trap checking version control
as we are not able to use vc in emacs do to not encripting passwords disaable the feature
#+BEGIN_SRC emacs-lisp :tangle no
...
* compilation
** scroll output.
setting it to t for now. should be set top 'first-error when the issues with xcelium compiles is handeled
#+BEGIN_SRC emacs-lisp
...
* configuring org-roame
** you need sqlite3 for it to work
#+BEGIN_SRC emacs-lisp
  (use-package sqlite3
    :ensure t)
#+END_SRC

** setting up org-roam it self
#+BEGIN_SRC emacs-lisp
  (use-package org-roam
    ...
    (advice-add 'wrap-fido-mode-for-this-function :around #'org-roam-node-find)
    (advice-add 'wrap-fido-mode-for-this-function :around #'org-roam-node-insert)
    :custom
...
** use fido as compleation in find and inset
#+BEGIN_SRC emacs-lisp
  (advice-add 'wrap-fido-mode-for-this-function :around #'org-roam-node-find)
  (advice-add 'wrap-fido-mode-for-this-function :around #'org-roam-node-insert)
#+END_SRC
...
** Use Org-agenda with org-roam (not functionell at this time)
Since org-agenda does not work to well with multipple files keeping a list of the org-roam files that has TODO's in it helps org-agenda speed up.

this is taken from this artical [[https://www.d12frosted.io/posts/2021-01-16-task-management-with-roam-vol5][Task management with org-roam Vol. 5: Dynamic and fast agenda]]

*** Check if org-rome file has project tag

#+BEGIN_SRC emacs-lisp :tangle no
  (defun check-org-roam-file-if-project-p ()
...
*** Add a tag to all org-roam files that has TODO's

#+BEGIN_SRC emacs-lisp :tangle no
  (defun update-org-roam-with-project-tag ()
    "Update PROJECT tag in the current buffer."
      (when (and (not (active-minibuffer-window))
               (check-for-org-roam-buffer-p))
      (let* ((file (buffer-file-name (buffer-base-buffer)))
             (all-tags (org-roam--extract-tags file))
             (prop-tags (org-roam--extract-tags-prop file))
             (tags prop-tags))
        (if (check-org-roam-file-if-project-p)
...
since all tags propagate to childs that creates extra mess that is not needed
...
We need to check for updates when storing files, if the change is a TODO.
The article also do say to update when finding a file, but I can not see the reasoin for this so commenting out until I become wise.

#+BEGIN_SRC emacs-lisp :tangle no
  ;(add-hook 'find-file-hook #'update-org-roam-with-project-tag)
  (add-hook 'before-save-hook #'update-org-roam-with-project-tag)
#+END_SRC

To only try this for org-roam buffers we have a heper function to detect org-roam buffers


#+BEGIN_SRC emacs-lisp :tangle no
  (defun check-for-org-roam-buffer-p ()
...
*** querry org-roam DB for all files with project tag

#+BEGIN_SRC emacs-lisp :tangle no
  (defun query-org-roam-db-for-project-files ()
...
*** for now create a seperate function for calling this org-agenda as you have old org agenda stuff
Move it over to use standard org-agenda at some point
(advice-add 'org-agenda :before #'vulpea-agenda-files-update)
(advice-add 'org-todo-list :before #'vulpea-agenda-files-update)

#+BEGIN_SRC emacs-lisp :tangle no
  (defun use-org-roam-files-for-org-agenda (&rest args)
    "Change the filelist used by org-agenda to the org-roam files for todo search"
    (interactive)
    (let ((org-agenda-files (query-org-roam-db-for-project-files)))
